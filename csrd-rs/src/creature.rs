...
pub fn load_creatures(file: &str, kind: &str) -> Vec<Creature> {
    ...
    for capture in creature_regex.captures_iter(&creatures) {
        let creature = Creature {
            ...
            options: capture.name("options").map(|s| s.as_str()).map(load_option_table).map(|t| RollTableBuilder::default().table(t).name(capture.name("optname").map(|s| s.as_str().trim().into())).build().unwrap()),
